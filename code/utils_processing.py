...
def get_subfolder_to_overlap(subfolders: List[Path],
    ...
    for subfolder in subfolders:
        subfolder_size, subfolder_num_images = get_folder_size_and_num_images(
            folder=subfolder)#返回文件和/10^6的值及文件个数
...
def gen_train_patches(input_folder: Path, output_folder: Path,
    ...
    subfolder_to_overlap_factor = get_subfolder_to_overlap(
        subfolders=subfolders, desired_crops_per_class=num_train_per_class)#{PosixPath('/disk1/zh..._train/a'): 1.0}
...
def balance_classes(training_folder: Path) -> None:
    ...
    subfolder_to_images = {
        ...
        for subfolder in subfolders
    }#{PosixPath('train_fol.../train/a'): [PosixPath('train_fol...304.jpeg'), PosixPath('train_fol...528.jpeg'),
    #得到各类下切片文件名，形成字典
    # Find the class with the most images. 得到最多切片数量3530
    biggest_size = max({
...
def produce_patches(input_folder: Path, output_folder: Path,
    ...
    image_locs = get_all_image_paths(
        master_folder=input_folder) if by_folder else get_image_names(
            folder=input_folder)#拿到该类所有图片名[PosixPath('2006799-2...5-03.png'), PosixPath('2006799-3...6-05.png'), PosixPath('2006799-4...0-04.png'), PosixPa
    outputted_patches = 0
    ...
    for image_loc in image_locs:
        #print(by_folder,input_folder.joinpath(image_loc))  False wsi_train/a/C3L-00009-21.svs
        image = imread(
            uri=(image_loc if by_folder else input_folder.joinpath(image_loc)))#(11520, 12416, 4)
        ...
        step_size = int(patch_size / inverse_overlap_factor)#所以inverse_overlap_factor是移动步幅的因子
