...
class Net(nn.Module):
    def __init__(self,num_layers,num_classes):
        ...
        # Regressor for the 3 * 2 affine matrix    zyxchange 3 * 3 to 52*52
        ...
        self.model_constructor = getattr(torchvision.models,f"resnet{self.num_layers}")
        self.model = self.model_constructor(num_classes=self.num_classes)
...
def create_model(num_layers: int, num_classes: int,
    ...
    #model_constructor = getattr(torchvision.models,"inception_v3" )  #3.9改为inception_v3尝试
    model_constructor = getattr(torchvision.models,f"resnet{num_layers}")# "alexnet"  zyx
    #model = model_constructor(num_classes=num_classes)

    model=Net(num_layers,num_classes)
    if pretrain:
...
def get_data_transforms(color_jitter_brightness: float,
    ...
    zyx chhange 448 to 224,can not certain 448 is set by who
    """
    return {
        "train":
...
def train_helper(model: torchvision.models.resnet.ResNet,
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for idx, (inputs, labels) in enumerate(dataloaders["train"]):
            train_inputs = inputs.to(device=device)#shape is [40,3,448,448]
            train_labels = labels.to(device=device)# shape is [40]
            optimizer.zero_grad()

            # Forward and backpropagation.
            with torch.set_grad_enabled(mode=True):
                train_outputs = model(train_inputs)#size 16,5     inception output has 2 parts ,first is auxi second is last layer output, shape is [40,6]
                __, train_preds = torch.max(train_outputs, dim=1)#this line and nextline inception using train_outputs[1]  resnet using train_outputs
        ...
        for idx, (val_inputs, val_labels) in enumerate(dataloaders["val"]):
            #val_inputs.resize_(16,3,448,448)
            val_inputs = val_inputs.to(device=device)
            val_labels = val_labels.to(device=device)

            # Feed forward.
            with torch.set_grad_enabled(mode=False):
                val_outputs = model(val_inputs)#shape is [40,6]
                #'''this two line inceptiopn no using [1]'''
...
def train_resnet(
    ...
    Args:
        ...
        save_interval: Number of epochs between saving checkpoints.
    """
    # Loading in the data.  下面参数分别为0.5 0.5 0.2 0.5 
    ...
    if resume_checkpoint:
        ...
        if num_classes != pretrained["fc.weight"].size(0):
            del pretrained["fc.weight"], pretrained["fc.bias"]
        #zyx end
        model.load_state_dict(state_dict=pretrained, strict=False)#zyx change ckpt["model_state_dict"] to pretrained,adding ", strict=False"
        # optimizer.load_state_dict(state_dict=ckpt["optimizer_state_dict"])  zyx note off
        # scheduler.load_state_dict(state_dict=ckpt["scheduler_state_dict"]) zyx  note off
        start_epoch = 0#ckpt["epoch"] zyx change
...
def get_predictions(patches_eval_folder: Path, output_folder: Path,
    ...
    for image_folder in image_folders:
        ...
        num_test_image_windows = len(dataloader) * batch_size#640

        # Load the image names so we know the coordinates of the patches we are predicting.
        image_folder = image_folder.joinpath(image_folder.name)#到Acinar/Acinar
        window_names = get_image_paths(folder=image_folder)#得到多有下面的jpeg文件绝对路径list
        ...
        with output_folder.joinpath(f"{image_folder.name}.csv").open(
            ...
            for batch_num, (test_inputs, test_labels) in enumerate(dataloader):
                batch_window_names = window_names[batch_num *
                                                  ...
                                                  batch_size + batch_size]
                #test_inputs shape is [40,3,448,448]  test_label len 40
                confidences, test_preds = torch.max(nn.Softmax(dim=1)(model(
                    test_inputs.to(device=device))),
                                                    dim=1)#长度都是40，分别为预测概率（预测类别对应）及预测类别
                #print('confidence,test_preds is ',confidences,test_preds).size()
                #print('test_inputs, test_labels is ',test_inputs[0], test_inputs[1],test_labels)
                #sys.exit(0)
                #zyx:3.2 get all predict P
                predict_P= nn.Softmax(dim=1)(model(test_inputs.to(device=device)))
                print(predict_P.size())
                for i in range(test_preds.shape[0]):
                    # Find coordinates and predicted class.
                    xy = batch_window_names[i].name.split(".")[0].split('_')#(";")
                    #print(f"{','.join([xy[0], xy[1], xy[2],f'{class_num_to_class[test_preds[i].data.item()]}', f'{confidences[i].data.item():.5f}'])}\n")
                    # writer.write(
                    #     f"{','.join([xy[0], xy[1], xy[2],f'{class_num_to_class[test_preds[i].data.item()]}', f'{confidences[i].data.item():.5f}'])}\n"
                    # )
                    writer.write(
                        f"{','.join([xy[0], xy[1], xy[2],f'{class_num_to_class[test_preds[i].data.item()]}', f'{confidences[i].data.item():.5f}'])}\t"#.item()
                    )
                    preP=[p.item() for p in predict_P[i].data]
                    writer.write(str(preP)+'\n')
